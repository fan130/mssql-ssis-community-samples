...
namespace Microsoft.Samples.SqlServer.SSIS.SharePointListAdapters
{
	...
	public class SharePointListDestination : PipelineComponent
	{
		...
		private void AddUserProperties()
		{
			...
			IDTSCustomProperty100 batchType = ComponentMetaData.CustomPropertyCollection.New();
		...
		public override DTSValidationStatus Validate()
		{
			...
			if (ComponentMetaData.AreInputColumnsValid == false)
			{
				...
				{
					foreach (IDTSInputColumnCollection100 collection in ComponentMetaData.InputCollection)
					{
		...
		private DTSValidationStatus ValidateSharePointColumns()
		{
			...
			var fieldNames = (from col in ComponentMetaData.InputCollection[0].ExternalMetadataColumnCollection.Cast<IDTSExternalMetadataColumn100>()
			...
			var mappedFields =
			   ...
			   join metaCol in ComponentMetaData.InputCollection[0].ExternalMetadataColumnCollection.Cast<IDTSExternalMetadataColumn100>()
		...
		/// <summary>
		/// Loads the column data into the dts objects from the datasource for columns
		/// </summary>
		...
		/// <param name="sharepointUrl"></param>
		/// <param name="listName"></param>
		private static void CreateExternalMetaDataColumns(IDTSInput100 input, string sharepointUrl, string listName)
		{
			...
			try
			{
				...
				foreach (var column in accessibleColumns)
				{
					...
					IDTSExternalMetadataColumn100 dtsColumnMeta = input.ExternalMetadataColumnCollection.New();
					...
					IDTSInputColumn100 foundCol =
						(from col in input.InputColumnCollection.Cast<IDTSInputColumn100>()
		...
		public override void ProcessInput(int inputID, PipelineBuffer buffer)
		{
			...
			if (!buffer.EndOfRowset)
			{
				...
				string infoMsg = string.Format(
					...
					resultData.Elements().Count() - errorRows.Count(),
					sharepointList,
					sharepointUrl,
